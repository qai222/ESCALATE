...
def box_int(b1, b2):
    ...
    for i in range(len(b1.T)):
        ...
        if (
            (b1.T[i][0] <= b1.T[i][1])
            and (b1.T[i][1] <= b2.T[i][0])
            and (b2.T[i][0] <= b2.T[i][1])
        ):
            return 0
        elif (
            (b1.T[i][0] <= b2.T[i][0])
            and (b2.T[i][0] <= b1.T[i][1])
            and (b1.T[i][1] <= b2.T[i][1])
        ):
            olap_cds.append([b2.T[i][0], b1.T[i][1]])
        elif (
            (b1.T[i][0] <= b2.T[i][0])
            and (b2.T[i][0] <= b2.T[i][1])
            and (b2.T[i][1] <= b1.T[i][1])
        ):
...
def sampleConcentrations(allowedHull, nExpts=96):
    ...
    while len(pts) < nExpts:
        rand_pt = [
            np.random.uniform(bbox[0][i], bbox[1][i]) for i in range(len(bbox[0]))
        ]
...
def sampleUntilSuccessful(corners, boundingCuboid):
    ...
    i = 0
    while i < 100000:
        sample = ConvexSolution(
            corners,
            np.array(
                [
                    np.random.uniform(boundingCuboid[i][0], boundingCuboid[i][1])
                    for i in range(len(boundingCuboid))
                ]
            ),
        )
        if sample.sum() > 0:
...
def generateExperiments(
    ...
    if excludedReagents is None:
        ...
        if len(list(nonzeroReagentsDef.values())[0]) > 3:
            ...
            int_hull = ConvexHull(
                convex_hull_intersection(a_hull.points, b_hull.points)
            )
            ...
            if int_hull.volume > 0:
                ...
                while len(pts) < nExpt:
                    rand_pt = [
                        np.random.uniform(bbox[0][i], bbox[1][i])
                        for i in range(len(bbox[0]))
                    ]
                    if (in_hull(np.array(total_hull.points), rand_pt)) and not (
                        in_hull(np.array(int_hull.points), rand_pt)
                    ):
                        pts.append(rand_pt)
...
def generateHitAndRunExperiments(
    ...
    boundingCuboid = box_int(
        np.array(bb_cds(hullCorners)),
        np.array([[0] * int(dimensionality), [maxMolarity] * int(dimensionality)]),
    )
    results = []
    i = 0
    while i < nExpt:
